    ...
    if (selectedMBTIs.length === 0) {
        ...
        for (let index in combinations[selectedMBTIs[0]].best) {
            if (selectedMBTIs[1] === combinations[selectedMBTIs[0]].best[index]) {
                messageBox.innerHTML = selectedMBTIs[0] + " 대 " + selectedMBTIs[1] + ": 매우 잘 맞는 조합입니다";
                found = true;
                break;
            }
        }
        if (found !== true) {
            for (let index in combinations[selectedMBTIs[0]].good) {
                if (selectedMBTIs[1] === combinations[selectedMBTIs[0]].good[index]) {
                    messageBox.innerHTML = selectedMBTIs[0] + " 대 " + selectedMBTIs[1] + ": 평균보다 더 잘 맞는 조합입니다";
                    found = true;
                    break;
                }
            }
        ...
        if (found !== true) {
            for (let index in combinations[selectedMBTIs[0]].bad) {
                if (selectedMBTIs[1] === combinations[selectedMBTIs[0]].bad[index]) {
                    messageBox.innerHTML = selectedMBTIs[0] + " 대 " + selectedMBTIs[1] + ": 평균보다 잘 안맞는 조합입니다";
                    found = true;
                    break;
                }
            }
        ...
        if (found !== true) {
            messageBox.innerHTML = selectedMBTIs[0] + " 대 " + selectedMBTIs[1] + ": 평균으로 맞는 조합입니다";
        }        
    }    
}
